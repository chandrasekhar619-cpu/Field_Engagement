...
const WORDS = [
  ...
  { word: "GRACE", note: "After your premium due date, every policy gives you a buffer window before it is treated as inactive — called the grace period. Under most plans: 15 days if you pay monthly, and 30 days for quarterly, half-yearly, or annual modes. Your cover stays fully active during this period even if you haven't paid yet. Miss it, and the policy lapses — though most plans allow revival by paying outstanding premiums with interest." },
  ...
  { word: "LOANS", note: "Once your policy acquires a surrender value — typically after 2 to 3 years for traditional plans — you can borrow against it. Most plans allow loans up to 90% of the surrender value, at interest rates that are generally lower than personal loans. Your life cover stays active while the loan is outstanding. One thing to keep in mind: any unpaid loan balance, principal plus interest, is deducted from the final maturity or death payout. Borrow if you need to, but factor in repayment." },
  ...
  { word: "RATES", note: "Bonus rates in PAR plans are declared annually by the insurer based on the participating fund's performance — not guaranteed in advance, and can go up or down year to year. Declared bonus rates are published annually by the insurer. For non-PAR guaranteed plans, the income or maturity rate is locked at policy inception — no surprises either way. For ULIPs, the relevant rate is the Net Asset Value of your chosen fund, recalculated every day as markets move." },
  ...
  { word: "AGENT", note: "Field agents are called Financial Life Consultants. Their role is to understand your existing cover, your income, your dependents, and your goals — and then recommend accordingly. A good Financial Life Consultant knows the difference between a PAR plan, a guaranteed plan, and a ULIP, and when each is appropriate. The quality of that first conversation has a direct bearing on whether the policy stays in-force for its full term — which is when it actually does what it was designed to do." },
